...
def combine_csv_files(input_directory):
    ...
    # Print input directory for debugging
    print(f"Input directory: {input_directory}")

    # Extract pod name from the input directory
    pod_name = os.path.basename(os.path.normpath(input_directory))
    print(f"Extracted pod name: {pod_name}")  # Debugging statement

    # Handle case where pod_name might be empty
    if not pod_name:
        print("Error: Pod name could not be determined from the input directory.")
        return
